...
router.get("/getToken/:token", (req, res, next) => {
    try {
        ...
        // here we have the email and password
        ...
        userData.findOne({ "account.email": email }, (error, data) => {
            if (error) {
                return next(error);
            } else if (data) {
                // reformatting the payload before encryption
...
const hashPassword = data.account.password;
...
router.post("/", (req, res, next) => {
    ...
    bcrypt.genSalt(saltRounds, function (err, salt) {
        bcrypt.hash(password, salt, function (err, hashedPassword) {
            ...
            // new payload used to match the schema created by the odm tool
